...
}).on('click', '.btn-remove', function(e)
{
 ...
 var memid = this.id;
                   ...
                   if (confirm("Sure! You want to delete this Question Search Keyword")==true) {
                    $.ajax({
        url: "keyword_of_process.php",
        type: "post",
        data: {del_keyword:memid} ,
...
return false;
    ...
    <div class="mx-auto mt-2"> <!-- Center the content horizontally and add top margin -->
        ...
        <div style="color:black; margin-top:20px; margin-left:8px; height: 35px" class=" form-control" name="question">
                             
                            <?php
                                        $psel = "SELECT * FROM `questions`";
                                        $pq1 = mysqli_query($conn, $psel);
                                        while ($prow = mysqli_fetch_assoc($pq1)) {if($prow['id'] == $id){?>
                                          <p> <?php echo $prow['questions']; ?></p>
                                    <?php }}?>
                          </div>
<!-- 
...
<div class="container p-2" style="border-radius:10px; background: #6190e8; /* fallback for old browsers */
...
<?php
                            $sender=$_GET['id']; 
        $query="SELECT * from search  WHERE q_id =$id";
    ...
    <div class="fvrduplicate row" style="margin-bottom:0px; margin-left:50px;">
        <div class="form-group9 col-12 col-sm-1 " style="color:black;"><p class='item_no' style="font-size:12px; font-weight:700;"><?php echo $sno; ?></p></div>
        <div class="form-group9 col-12 col-sm-1 new3 " style= "width:740px; margin-left:-30px;">
            <input style="height:30px;" type="text" class="form-control chk-error" required row-id='<?php echo $row['s_id']?>' id="title_0" name="title[0]" placeholder='Keyword' value="<?php echo $row['kewords']?>"  id="title_<?php echo $row['s_id']?>" name="etitle[<?php echo $row['s_id']?>]">
        </div>
       
        <div class="form-group9 col-12 col-sm-1 new3" style="margin-left:-23px;">
            <button style="width:100px;" class="btn btn-danger btn-remove pro-row rounded-pill add keybutton dontremove" type="button"  id="<?php echo $row['s_id'] ?>" row-id='<?php echo $row['s_id'] ?>'><i class="fa fa-minus" style="font-size:9px; margin-left:-5px;"></i></button>
        </div>
    </div>
    <?php
         $sno++;
    ...
    <div class="fvrduplicate row" style="margin-bottom:0px; margin-left:50px;">
        ...
        <div class="form-group9 col-12 col-sm-1 new3" style="margin-left:-23px;">
            <button style="width:100px;" class="btn btn-primary btn-add pro-row rounded-pill add keybutton dontremove" type="button" id="r_0"><i class="fa fa-plus" style="font-size:9px; margin-left:-5px;"></i></button>
        </div>
    </div>
...
<div class="text-center btns" style="margin-top:20px;">
    <button style="width:100px; font-size:12px; font-weight:700; margin-left:-5px;" row-id="0" class="btn btn-primary rounded-pill" type="submit" name="edit_question_submit">Submit</button>
</div>
